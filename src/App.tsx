...
export function App() {
  ...
  return (
    <div className="app-shell" style={{ width: '100%', height: '100%', minHeight: 0, position: 'relative', background: 'linear-gradient(180deg,#12161f 0%,#0c0f16 60%,#090b10 100%)', display: 'flex', flexDirection: 'column', color: '#F6EBDD', overflow: 'hidden' }}>
      ...
      {ui.settingsOpen && accountOpen && (
        <DialogSurface label={accountCopy.title} onClose={() => setAccountOpen(false)} style={{ position: 'fixed', inset: 0, zIndex: 92, background: '#0b0e16', display: 'flex', flexDirection: 'column' }}>
      ...
      {showBattleIntro && (
        <button
          ...
          style={{ position: 'fixed', inset: 0, zIndex: 1000, width: '100%', border: 0, color: 'inherit', background: 'rgba(7,9,13,.78)', backdropFilter: 'blur(4px)', WebkitBackdropFilter: 'blur(4px)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', padding: 24, textAlign: 'center' }}
        >
