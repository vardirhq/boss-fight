...
| BF-013 | In progress | Adult email validation, explicit production CORS origins, opt-in proxy trust, defensive API headers, non-root containers, account deletion UI, active-session management, bounded absolute/idle token lifetimes, and non-disclosing health responses are implemented. Email verification and password recovery remain. |
