...
| BF-005 | In progress | A bilingual privacy notice, versioned child authorization, household export, transactional child/household/adult-account erasure, transient/active-data lifecycle rules, real-PostgreSQL erasure coverage, verified backup creation, bounded backup expiry, and an isolated restore/erasure drill are implemented. Qualified counsel review, processor/hosting disclosures, the external deletion-request register, deletion SLA confirmation, and actual quarterly drill records require organization/operator action. |
| BF-006 | In progress | Unit and contract suites cover game rules, recurrence, encounters, progression, seed integrity, SQLite fallback failures, sync recovery, reward integrity, governance, and accessibility. Real-PostgreSQL API tests cover schema validation, sessions, recovery, privacy lifecycle, pagination, and concurrent final blows. A native Android 35 emulator journey now covers accessible onboarding, SQLite-backed state after offline/process restarts, and version-code upgrade retention; remediation awaits its first green CI execution. |
...
| BF-009 | Remediated | All five event streams use durable independent cursors and server-enforced bounded pages that the client drains to completion. Reward status transitions receive fresh sequences, avatar bytes are hash-gated, and mutable configuration is omitted when the client's durable cache matches the server revision. |
...
| BF-019 | Remediated | The server entry point is now a small composition root. Authentication/account, household/access/privacy/governance, invitation/pairing, gameplay, synchronization, operational, error, observability, retention, schema, projection, credential, stable-ID, and boss-scheduling responsibilities are isolated behind explicit modules and independent contract/domain tests. |
