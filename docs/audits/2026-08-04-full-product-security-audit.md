...
| BF-006 | In progress | Game rules, recurrence, elite/rare encounters, progression, seed integrity, bootstrap/projection, sync queue, reward-integrity, household-governance, PostgreSQL privacy-erasure, and concurrent-final-blow integration tests now run in CI. The obsolete PWA service worker/manifest pipeline has been removed to match the native-only product scope. Broader database integration, native Android offline/restart/upgrade coverage, and UI/accessibility journeys remain. |
