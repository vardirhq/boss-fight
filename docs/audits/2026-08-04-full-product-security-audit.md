...
| BF-013 | In progress | Adult email validation, enumeration-resistant password recovery, explicit production CORS origins, opt-in proxy trust, defensive API headers, non-root containers, account deletion UI, active-session management, bounded absolute/idle token lifetimes, and non-disclosing health responses are implemented. Email verification remains. |
