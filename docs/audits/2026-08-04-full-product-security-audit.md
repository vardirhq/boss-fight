...
| BF-006 | In progress | Unit and contract suites cover game rules, recurrence, encounters, progression, seed integrity, SQLite fallback failures, sync recovery, reward integrity, governance, and accessibility. Real-PostgreSQL API tests cover schema validation, sessions, recovery, privacy lifecycle, pagination, and concurrent final blows. A native Android emulator journey now covers accessible onboarding, SQLite-backed state after offline/process restarts, and version-code upgrade retention; remediation awaits its first green CI execution. |
