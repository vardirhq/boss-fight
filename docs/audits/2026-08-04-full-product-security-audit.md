...
| BF-008 | Remediated | Android credentials use Keystore-backed AES-GCM storage with legacy-token migration and browser-record scrubbing; a restrictive CSP limits script and connection origins in PR #41. Active-session listing and individual revocation are now implemented; token lifetime improvements remain under BF-013. |
...
| BF-013 | In progress | Adult email validation, explicit production CORS origins, opt-in proxy trust, defensive API headers, non-root containers, account deletion UI, active-session management, and non-disclosing health responses are implemented. Email verification, password recovery, and token-lifetime improvements remain. |
