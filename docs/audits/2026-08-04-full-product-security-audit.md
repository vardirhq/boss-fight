...
| BF-005 | In progress | A bilingual privacy notice, versioned child-authorization records, household export, transactional child erasure, and automated retention for transient operational records/deleted avatars are implemented. Counsel review, adult/household erasure, active-data and backup lifecycle rules, processor details, and real-database/backup-erasure testing remain. |
