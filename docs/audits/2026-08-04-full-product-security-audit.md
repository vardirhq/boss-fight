...
| BF-013 | In progress | Adult email validation, explicit production CORS origins, opt-in proxy trust, defensive API headers, non-root containers, account deletion UI, and non-disclosing health responses are implemented. Email verification, password recovery, and user-facing session management remain. |
