...
| BF-009 | In progress | The client durably merges household event streams and sends independent server-sequence cursors; reward status transitions now receive fresh sequences, so all five event streams avoid repeated full-history downloads. Avatar bytes are hash-gated and retained only while their hashes match. Bounded pagination and revision-gated mutable configuration remain. |
