...
Capacitor and its Android platform are pinned project dependencies. Packaging
also registers `@aparajita/capacitor-secure-storage`, which stores online tokens
with AES-GCM using a key generated by Android Keystore. Do not replace the pinned
dependency installation with ad-hoc `--no-save` Capacitor packages in CI.
